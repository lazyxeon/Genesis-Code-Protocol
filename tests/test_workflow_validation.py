    ...
    def test_no_duplicate_checkout_actions(self, workflow_files):
        ...
        for workflow_file in workflow_files:
            ...
            for job_name, job_config in workflow.get('jobs', {}).items():
                ...
                assert checkout_count <= 2, f"Job '{job_name}' in {workflow_file} has {checkout_count} checkout actions (max 2 recommended)"
        ...
        for workflow_file in workflow_files:
            ...
            for job_name, job_config in workflow.get('jobs', {}).items():
                if 'timeout-minutes' in job_config:
                    ...
                    assert timeout <= 360, \
                        f"Job '{job_name}' in {workflow_file} has excessive timeout: {timeout} minutes (max 360 recommended)"
